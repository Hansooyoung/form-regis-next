...
export default function RegisterForm() {
  ...
  // Buat memoized fetch functions untuk RegionSelect
  const getRegenciesMemoized = useCallback(() => 
    getRegencies(formData.province), [formData.province]);
  
  const getDistrictsMemoized = useCallback(() => 
    getDistricts(formData.regency), [formData.regency]);
    ...
    <AuthForm onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <textarea
          ...
          onChange={handleChange}
          rows={3}
          className={`mt-1 block w-full border ${errors.address ? 'border-red-500' : 'border-gray-300'}  text-gray-700 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm`}
        />
      ...
      <div>
        <button
          type="submit"
          disabled={!dataValid}
          className={`w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${dataValid ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-400 cursor-not-allowed'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500`}
        >
          Daftar
        </button>
      </div>
